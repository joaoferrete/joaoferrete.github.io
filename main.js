...
(function () {
    ...
    var i18nEls = Array.prototype.slice.call(document.querySelectorAll("[data-en][data-pt]"));
    ...
    function setLang(lang) {
        ...
        // toggle visual state
        if (langToggle) {
            Array.prototype.forEach.call(langToggle.querySelectorAll(".lang-toggle__opt"), function (opt) {
    ...
    if (svg) {
        ...
        nodes.forEach(function (n, idx) {
            ...
            for (var k = 0; k < 2; k++) {
                ...
                if (!edgeSet[key]) { edgeSet[key] = true; edges.push([idx, j]); }
            }
        });
        ...
        var edgeEls = edges.map(function (e) {
            ...
            line.setAttribute("x1", nodes[e[0]].x); line.setAttribute("y1", nodes[e[0]].y);
            line.setAttribute("x2", nodes[e[1]].x); line.setAttribute("y2", nodes[e[1]].y);
        ...
        var startNode = 0, bestD = Infinity;
        nodes.forEach(function (n, idx) {
            var dx = n.x - SIZE * 0.72, dy = n.y - SIZE * 0.42;
            var d = dx * dx + dy * dy;
            if (d < bestD) { bestD = d; startNode = idx; }
        });

        // Build a walk through the graph following existing edges (BFS-ish).
        function buildWalk() {
            var visited = {}, walk = [startNode], cur = startNode;
            visited[startNode] = true;
            for (var step = 0; step < NODE_COUNT * 2; step++) {
                var opts = edges
                    .filter(function (e) { return e[0] === cur || e[1] === cur; })
                    .map(function (e) { return e[0] === cur ? e[1] : e[0]; })
                    .filter(function (n) { return !visited[n]; });
                if (!opts.length) {
                    var unseen = nodes.map(function (_, idx) { return idx; })
                        .filter(function (idx) { return !visited[idx]; });
                    if (!unseen.length) break;
                    cur = unseen[0]; visited[cur] = true; walk.push(cur);
                    continue;
                }
                cur = opts[Math.floor(Math.random() * opts.length)];
                visited[cur] = true; walk.push(cur);
            }
            return walk;
        }
        ...
        if (prefersReduced) {
            ...
            var nb = edges.filter(function (e) { return e[0] === startNode || e[1] === startNode; })[0];
            if (nb) nodeEls[nb[0] === startNode ? nb[1] : nb[0]].classList.add("is-active");
            return;
        }
        ...
        function startSegment(ts) {
            var a = walk[segIndex], b = walk[segIndex + 1];
            segStart = ts;
            segLen = dist(a, b);
            // overlay line that we "draw" via dashoffset
            segEl = document.createElementNS(SVG_NS, "line");
            segEl.setAttribute("x1", nodes[a].x); segEl.setAttribute("y1", nodes[a].y);
            segEl.setAttribute("x2", nodes[b].x); segEl.setAttribute("y2", nodes[b].y);
